[submission (1).md](https://github.com/user-attachments/files/30271747/submission.1.md)
# บันทึกการแก้โจทย์
...
```text
เริ่มด้วยการรับค่า input มา บรรทัดแรกคือ คะแนนรวมของคนสามคน และบรรทัดที่สองเป็นคะแนนของคนที่ได้คะแนนสูงสุด แทนด้วยตัวแปร score และ top ตามลำดับ
เราต้องหาคะแนนที่น้อยที่สุดของแต่ละรอบ เลยเอาคะแนนทั้งหมด - คะแนนสูงสุดเป็น score - top แทนด้วยตัวแปร left
หลังจากนั้น เราจะได้คะแนนรวมของสองคนที่เหลือ ซึ่งเราต้องหาว่าคะแนนที่น้อยสุดเป็นเท่าไหร่ เลยให้คะแนนสองคนที่เหลือ มีคนนึงคะแนนเยอะเท่าคะแนน top เป็น left - top และเราก็ได้คะแนนที่น้อยที่สุดแทนด้วยตัวแปร minimum
ซึ่งเราต้องกำหนดว่าคะแนนที่น้อยที่สุดต้องเป็นจำนวนจริงที่ไม่มากกว่าคะแนน top และไม่ติดลบ โดยใช้ if elif กำหนดเงื่อนไข
และใช้ if else กำหนดเงื่อนไขจากโจทย์ว่า ถ้าคะแนนสูงสุด - คะแนนต่ำสุด > 2 จะเป็น surprising else เป็น not surprising
```
...
```text
รับค่า input ตามโจทย์กำหนด
หาค่าเฉลี่ย
เอาคะแนนสูงสุด - ค่าเฉลี่ย 
```
...
```text
คล้ายแผนเดิม แต่จากใช้ค่าเฉลี่ย เปลี่ยนมาใช้ค่าที่เหลือจากการลบคะแนนสูงสุดออกไป ให้ได้คะแนนที่น้อยสุดจริงๆไม่ใช่ค่าเฉลี่ย
```
...
```text
เป็นการตรวจสอบกำหนดเงื่อนไขเบื้องต้น
```
