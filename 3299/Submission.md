...
```text
รับ input คือ l และ n กำหนดให้มีตัวแปรดังนี้
step = L (จำนวนช่องในแนวตั้งเริ่มต้นของแถบนั้น)
stop = N จำนวนช่องดอกไม้ทั้งหมดที่เราปลูกไป
n = เลขแถบปัจจุบัน
total = จำนวนช่องที่ปลูกไปแล้วทั้งหมด
bottom = ความยาวของด้านที่เพิ่มขึ้นตามหมายเลขแถบ
...
print(n)
 ...
 ใข้ while < stop เพื่อวนลูปบวกเพิ่มทีละแถบ ตราบใดที่จำนวนช่องรวมยังไม่ถึง N
n += 1 ขยับไปคำนวณแถบถัดไปเรื่อยๆ
เราจะคำนวณจำนวนช่องตรงยอดสามเหลี่ยมด้านบนก่อน 
for i in range(step, 0, -1):
        total += i
เช่น ถ้า L=3 จะบวกเพิ่ม 3 + 2 + 1 = 6 ช่อง
หลังจากนั้นคำนวณจำนวนช่องตรงพื้นที่สี่เหลี่ยมด้านล่าง แล้ว อัปเดตความยาวสะสมไว้ใช้ในรอบถัดไปแถบที่ n จะมีความยาวสะสมเก็บไว้สำหรับแถบถัดไปเท่ากับ L × n
```
