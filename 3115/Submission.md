[submission.md](https://github.com/user-attachments/files/31386919/submission.md)
# บันทึกการแก้โจทย์
...
```text
รับ input มาเป็นจำนวนร้านที่จะเช็ค = num, _ = จำนวนเวลาที่จะเช็ค
ให้ day เป็น list ไว้เช็ค โดยคูณ 1441 เพราะ 1 วันมี 1440 นาที อยากให้มีลิสต์ 1440 ช่องไว้เช็ค ให้ทุกช่องเป็น 0 รอนับ
ทำ loo[p รับเวลาปิดเปิดของแต่ละร้าน for k in range(num)
รับ input เวลาปิดเปิดของร้านชื่อ start, stop
ตั้งตัวแปร minute วนในช่วงเวลาที่ร้านเปิด เพราะให้เช็ค เริ่มเวลาที่ร้านเปิด หยุดเวลาที่ร้านปิด
ถ้า minute ก็รันตามเวลาเปิดปิด ให้เวลานับ 1 เวลาร้านเปิด เช่น day[600] ก็ดูว่านาทีที่ 600 มีร้านไหนเปิดบ้าง ถ้าเปิดก็ += 1 มันจะนับ จากลิสต์ช่องที่เป็น 0 ก็จะเป็น 1 เพิ่มมาตามที่เวลาเปิดปิด
บรรทัดสุดท้ายรับ input เวลาที่เราอยากจะเช็ค
ใช้ลูป i วนตามเวลาที่อยากเช็ค
ใช้ result.append(day[i]) วนสะสมร้านไปเรื่อยๆ
print คำตอบ ใช้ * เพื่ออให้ตำตอบเว้นกัน
```
...
```text
รับ input
ทำเป็นลูปมี count =0 พอวนเจอตรงก็ count += 1 ไปเรื่อยๆ
print count
```
...
```text
ยังใช้ loop เหมือนเดิม แต่เปลี่ยนมาใช้ list แทนเพื่อappend ค่าไปต่อแถวหลังสุดเรื่อยๆ แต่ใช้ list เป็นช่องๆ 1440 ช่องแทน เวลามีร้านเปิดก็ให้ช่อง += 1
```
