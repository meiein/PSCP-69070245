[submission (5).md](https://github.com/user-attachments/files/30744197/submission.5.md)
# บันทึกการแก้โจทย์
...
```text
รับ input เข้ามา ใช้ split เพื่อแยกข้อมูลที่อยู่บรรทัดเดียวกัน แทนด้วยตัวแปร place และแทนน้ำหนักด้วย w 
กำหนดค่าว่า place ตัวแรก [0] เป็นสถานที่หนึ่งไปอีกที่หนึ่งตามที่โจทย์กำหนดไว้ และ ให้ print ราคาออกมาตามโจทย์ ใช้ ค่าธรรมเนียมเริ่มต้น + ราคาตามน้ำหนัก ถ้า place ไม่มีในเงื่อนไขที่กำหนด print Error และโจทย์ต้องการทศนิยม 2 ตำแหน่ง ใช้ :.2f
```
...
```text
รับค่า input ใช้ split แยกข้อมูลในบรรทัดเดียวกัน
กำหนดเงื่อนไขตามโจทย์กำหนด และคิดราคาออกมา
หากไม่ตรงกับเงื่อไขใดเลย print Error
```
...
```text
ใช้แบบวิธีแรก ไม่มีอะไรเพิ่มเติม
```
...
```text
ดูว่าหากเงื่อนไขไม่ตรงจะ print Error มั้ย
```
