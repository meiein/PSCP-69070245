...
```text
รับ input 2 บรรทัด เป็น r1, g1, b1 และ r2, g2, b2
แบ่งโดยใช้ .split() และแปลงให้เป็น int เพื้อใช้คิดเลข ได้เป็น
r1, g1, b1 = map(int, input().split())
r2, g2, b2  = map(int, input().split())

คาค่าเฉลี่ยของแต่ละสี เอาค่าสีมา + กัน  หาร 2
และ print ค่าที่ได้ออกมา
r = int((r1 + r2) /2)
g = int((g1 + g2) /2)
b = int((b1 + b2) /2)
print(f"{r} {g} {b}")
...
```text
รับinput แปลงเป็น int
หาค่าเฉลี่ยของแต่ละสี
print ค่าที่หาได้
```
