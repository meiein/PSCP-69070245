[submission (7).md](https://github.com/user-attachments/files/32458442/submission.7.md)
# บันทึกการแก้โจทย์
...
```text
รับข้อความจากโจทย์ ซึ่งการที่เราจะทำกรอบที่ใหญ่ที่สุดเราต้องหาให้ได้ก่อนว่าข้อความไหนยาวที่สุด
ใช้ลูปวน รับ input ใส่ไปในลิสต์ตัวเอง
ใช้ len นับจำนวนความยาวข้อความ และหาข้อความที่ยาวที่สุด ถ้าข้อความที่รับมายาวกว่า ก็แปลงเป็น maximum = ข้อความที่พึ่งรับไปเรื่อยๆ ได้เป็น
for _ in range(5):
    text = input().strip()
    ans.append(text)
    if len(text) > maximum:
...
กำหนดให้กรอบ คือ  maximum +4  เพราะข้อความยาวสุดมันต้องเว้นเป็น  "* ข้อความ * "จะเห็นว่ารสวมทั้งช่องว่างและ * จะรวมได้เป็น 4

หลังจากนั้น print("*" * กรอบที่คิดไว้) 
หลังจากนั้น เอาข้อความที่เราใส่ลิสต์ไว้ออกมา print เรื่อยๆโดยจัดให้ชิดซ้าย เว้นไป max ทีได้
for text in ans:
...
รับ input 
ใส่ลิสต์
หาข้อความที่ยาวที่สุด
print กอรบ
print ข้อความพร้อมกรอบ
```
