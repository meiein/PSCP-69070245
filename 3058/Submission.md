[submission (9).md](https://github.com/user-attachments/files/31376524/submission.9.md)
# บันทึกการแก้โจทย์
...
```text
รับ input ตามโจทย์ อิฐก้อนเล็ก = a อิฐก้อนใหญ่ = b อยากต่อให้ได้เท่าไหร่ = goal
หลังจากนั้นโจทยื์กำนดว่าต้องใช้อิฐก้อนใหญ่ให้หมดก่อน เลยกำหนดตัวแปรมาใหม่ชื่อ use_b 
โดยที่ใช้ min(b , goal // 5) เพราะเราจะหาว่าอะไรน้อยกว่ากัน อิฐที่โจทย์ให้มา หรือ การเอา goal // 5 (5 คือขนาดของอิฐ เช่น goal 14 คิดเลขออกมาได้ 2 แต่โจทย์ให้ b มา 1 ก็ต้องเอา 1 หรือก็คือเอาค่าที่น้อยที่สุด)
หลังจากนั้นดูว่าเหลือต้องใช้อิฐก้อนเล็กอีกเท่าไหร่ ได้ left = goal - use_b * 5 ก็คือเอา goal - จำนวนอิฐใหญ่ว่าใช้ไปเท่าไหร่ *5 มาจากขนาดของอิฐ เช่น ได้ use_b เป็น 2 ก้อนก็จะเป็น 10 นิ้ว
กำหนดเงื่อนไขว่าถ้า a >= goal  ให้ print(left) แต่ถ้า else กรณี a < goal ก็คืออิฐมีไม่พอ print("-1")
```
...
```text
เปลี่ยนวิธีคิด use_b เพราะวิธีคิดแบบแรกไม่ครอบคลุม เปลี่ยนใช้ min หาค่า b ที่น้อยที่สุดแทน
```
...
```text
ดู if else ถ้า  a น้อยกว่า จะถูกเงื่อนไขไหม
```
...
```text
ดูว่าถ้าไม่ต้องใช้ a เลยจะถูกไหม
```
...
```text
ช่วยอธิบายคอนเซ็ปต์ของโจทย์ว่ามีแนวคิดยังไง
```
