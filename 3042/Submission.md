[submission.md](https://github.com/user-attachments/files/30167088/submission.md)
# บันทึกการแก้โจทย์
...
```text
เริ่มด้วยการรับ input เข้ามาใช้แทนด้วยตัวแปร num 
และต้องการให้ตัวเลขวนไปเรื่อยๆเลยใช้ for i in range ซึ่ง start คือ input ที่เรารับเข้ามา และ stop ที่ -1 (เพราะยังอยาก output มี 0 อยู่) และ step คือ -1 เหมือนกัน (เพราะอยากให้เลขนับถอยหลัง) ได้เป็น for i in range(num, -1, -1)
หลังจากนั้นตั้งเงื่อนไขว่า ถ้า i หาร 10 แล้วเศษออกมาเป็น 0 ( i % 10 == 0) ให้ print i ออกมา จะได้เป็นทุกๆครั้งที่เลขหาร 10 ลงตัว จะ print เลขนั้นออกมา เช่น 89 ก็ลดที 1 เป็น 88 แต่ยังหารไม่ลงตัวก็จะข้ามไป จนถึง 80 ซึ่งหาร 10 ลงตัว เลย print 80 ออกมา
หลังการกำหนดเงื่อนไขเสร็จก็ให้ print i ออกมา โดยที่มี end = " " เพื่อเว้นเลขในบรรทัดเดียวกัน
```
...
```text
1 รับ input 
2 ใช้ if elif else กำหนดเงื่อนไขตามโจทย์
```
...
```text
คล้ายแผนแรกแต่ใช้ for i in range (start, stop, step) เพิ่มเข้ามา
```
...
```text
เพราะเป็นเงื่อนไขปกติตามโจทย์เบื้องต้น
```
