[submission (3).md](https://github.com/user-attachments/files/31747114/submission.3.md)
# บันทึกการแก้โจทย์
...
```text
รับ input ชื่อ start stop
สร้างลิสต์ไว้รอ ชื่อ ans
ใช้ลูปวนตัวเลขตาม input ที่รับมา for num in range(start stop+1)
กำหนดเงื่อนไข if num < 2 เพราะเลขจำนวนเฉพาะเริ่มที่ 2
ให้ primes = True
กำหนดลูปในลูปขึ้นมาเช็คว่าหารด้วยเลขอื่นจะลงตัวไหม เพราะเลขจำนวนเฉพาะหารลงแค่ 1 กับตัวมันเอง ได้  if not num % i โดย num คือเลขที่วนไปเรื่อยๆของเรา ส่วน i คือเลขที่น้อยกว่า เช่น range(2, 5) มีตัวเลข i = 2, 3, 4 ใช้เลขที่น้อยกว่าหารดูว่าหารลงตัวมั้ย ถ้าลงแสดงว่าไม่ใช่ ได้เป็น primes = False และออกจากลูป
ถ้า primes ยังเป้นจริง จะให้แทรก num ไปในลิสต์ ans ที่สร้างไว้ใช้ ans.append(num)
หลังจากนั้นเช็คว่าในลิสต์ ans มีคำตอบมั้ย ถ้ามี print ออกมา
และ print("Total primes:", len(ans)) โดย len นับสมาชิกในลิสต์ของ ans
```
...
รับ input 
กำหนดค่าจำนวนเฉพาะ
วนลูป ถ้าเลขในลูปเจอค่า ให้นับ
print ค่าที่นับ
print "Total primes:"
```
...
```text
เปลี่ยนจากการกำหนดค่าจำนวนเฉพาะมาเลยมาเป็นคิดเลขว่าเอาเลขมาหารแล้วหารลงวตัวไหม ถ้าหารลงจะไม่ใช่ และใช้ len นับสมาชิกในลิสต์
```
...
```text
ดูว่าถ้าไม่มีสมาชิกในลิสต์จะ print ออกมาถูกไหม
```
...
```text
ช่วยอธิบายวิธีการหาจำนวนตเฉพาะว่าจำนวนเฉพาะจะมีได้เมื่อ หารได้ดด้วย 1 และตัวมันเท่านั้น
```
