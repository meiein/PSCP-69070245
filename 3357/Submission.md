[submission (6).md](https://github.com/user-attachments/files/32335592/submission.6.md)
# บันทึกการแก้โจทย์
...
```text
รับ input ชื่อ totalg ไว้วนจำนวนส่วนสูงยีราฟตั้งหมดด totalg ตัว
สร้าง glist ไว้ใส่ส่วนสูงของยีราฟ
กำหนดให้ count = 0 ไว้นับ ได้
glist = []
count = 0
totalg = int(input())

วนลูปรับส่วนสูงยีราฟแล้วใส่ไว้ใน glist ได้ 
for i in range(totalg):
    height = int(input())
    glist.append(height)

กำหนดเงื่อนไขว่าถ้าจำนวนยีราฟ คือ 1 ให้ print 1 เพราะไม่มีใครแย่งอาหาร
เงื่อนไขอื่นคือ ถ้า i = 0 หรือก็คือยีราฟตัวตำแหน่งซ้ายสุด มากกว่าตัวขวา count += 1
ตัวขวาสุด หรือตัวที่  totalg - 1 มากกว่าตัวซ้ายของมัน count += 1
ส่วนตัวตรงกลางที่เหลือให้เทียบทั้งตัวซ้ายและขวา ถ้ามากกว่า coun += 1 print คำตอบได้เป็น
if totalg == 1:
    print(1)
else:
    for i in range(totalg):
...
รับ input
วนลูปใส่  []
กำหนดเงื่อนไขดูตำแหน่งแต่ละตัวมากกว่าตัวข้างๆไหม
ปริ้น count 
```
