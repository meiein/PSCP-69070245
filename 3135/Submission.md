[submission (2).md](https://github.com/user-attachments/files/31600652/submission.2.md)
# บันทึกการแก้โจทย์
...
```text
รับ input คือ  N จำนวนคนทั้งหมด K จำนวนคนที่จะถูกส่งของขวัญถัดไป T คนขโมย
โจทย์ให้หาคนที่ได้ดูของขวัญก่อนที่จะถูกขโมยหรือกลับไปคนที่ 1
 กำหนดให้  gift = 1 เพราะของขวัญเริ่มคนที่ 1
ใช้ while True ในขณะที่เงื่อนไขยังถูกจะไม่ออกจากลูปและ count += 1 ไปเรื่อยๆ
กำหนดตัวแปร next_person = ((gift -1 + K) % N) + 1 มาจาก
 - 1 เปลี่ยนเลขคน 1 ถึง N ให้เป็น 0 ถึง N-1 เพื่อหารเอาเศษ
+ K ข้ามไปอีก K คน
+ 1 คืนค่าเลขคนจริง 1 ถึง N
ถ้า  gift == T คือถ้าของขวัญ อยู่กับขโมย จะจบลูป
ถ้า next person == 1 หรือคือวนกลับมาคนที่ 1 จบลูป
โดยกำหนดให้ gift == next person คือให้ของขวัญอยู่กับใครล่าสุด เช่น เริ่ม 1 ข้ามไป 2 gift จะอยู่กับคนสาม
print count
```
...
```text
รับ input
กำหนดเงื่อนไข ใช้ while ถ้าเงื่อนไข ตรงกับคนแรก หรือของขวัญตรงกับขโมยให้ออกจากลูป
ปริ้น count
```
...
```text
เหมือนแผนแรก มีตัวแปร next personเพิ่มขึ้นมา
```
