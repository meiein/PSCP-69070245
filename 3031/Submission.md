...
```text
รับ input มาเป็นน้ำหมึก กับจำนวนบ้านคน แทนตัวแปร ink, n
เนื่องจากจำนวนบ้านแต่ละทีไม่เท่ากันเลยใช้วน for i in range(n) วนจำนวนบ้าน n รอบ
พอวนแล้วรับ input พิกัดบ้านหลังแรกมาคิดเลข
น้ำหมึกกระจายเป็นพื้นที่วงกลม ซึ่งจะต้องหาพื้นที่วงกลมบ้านพิกัดที่เราอยู่ เพื่อเอามาหารกันให้ได้ออกมาเป็นเวลา
สูตรพื้นที่วงกลมคือ pi * (r  ** 2) เพราะงั้นต้องหา r
หา r จากบทพีทาโกรัสเป็น r**2 = x**2 + y ** 2
ได้ค่า r เอาแทนค่าพื้นที่วงกลมใช้ค่า pi 3.1416 ตามโจทย์กำหนด
ถ้าคิดเลขได้ 0 ให้คำตอบเป็น 0
ถ้าได้ค่าปกติก็เอาค่ามาปัดขึ้น math.ceil
```
...
```text
คล้ายอันเดิม แต่ r **2  เปลี่ยนมาใช้บทพีทาโกรัสเพราะพิกัดบ้านหาจากสามเหลี่ยมในวงกลมได้
```
...
```text
เป็นการตรวจสอบเงื่อนไขเบื้องต้น
```
...
```text
ทดสอบการหารด้วยตัวเลข S สูงสุด
```
...
```text
เพื่อนช่วยอธิบายโจทย์ให้ฟัง และแนววิธีการคิดหาพื้นที่วงกลมตามพิกัด
```

คุณยังทำอะไรด้วยตนเอง

```text
ยังเขียนโค้ดด้วยตัวเองตามความเข้าใจจากที่เพื่อนช่วยสอน
```
